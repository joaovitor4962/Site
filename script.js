...
const CONFIG = {
  ...
  EMAILJS_PUBLIC_KEY: "AkmRbbP2sZ2SFfDs-",
...
const EmailManager = {
  ...
  sendContact: (form) => {
    if (typeof emailjs === 'undefined') {
      NotificationManager.toast('❌ Serviço de email não disponível', 'error');
      return;
    }

    emailjs.sendForm(CONFIG.SERVICE_ID, CONFIG.TEMPLATE_ID, form)
      .then(() => {
        NotificationManager.toast('✅ Mensagem enviada com sucesso!', 'success');
        form.reset();
      })
      .catch((err) => {
        NotificationManager.toast('❌ Erro ao enviar mensagem', 'error');
        console.error('Erro no email:', err);
      });
  }
};
